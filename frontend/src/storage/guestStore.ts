...
async function readAll<T>(storeName: string): Promise<T[]> {
  if (!dbAvailable()) return storeName === MEALS ? [...fallbackMeals.values()] as T[] : [...fallbackGoals.values()] as T[]
...
async function put(storeName: string, value: unknown): Promise<void> {
  if (!dbAvailable()) {
    ...
    else fallbackGoals.set((value as StoredGoal).effectiveDate, value as StoredGoal)
    return
  }
  const db = await openDb()
...
export async function createGuestMeal(food: Food, amountG: number, localDate: string, mealCategory: MealCategory): Promise<Meal> {
  ...
  const meal: Meal = { id, foodId: food.id, consumedAt: now, localDate, timezone: 'Europe/Budapest', amountG, mealCategory, calculatedCarbsG: carbs, snapshot: snapshot(food), createdAt: now, updatedAt: now }
...
export async function exportGuestData(): Promise<{ meals: Meal[]; goals: StoredGoal[] }> {
  return { meals: await readAll<Meal>(MEALS), goals: await readAll<StoredGoal>(GOALS) }
}
