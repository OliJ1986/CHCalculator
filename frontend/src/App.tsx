...
import { createCustomFood, createPlan, createRecipe, createShopping, deleteCustomFood, deletePlan, deleteRecipe, deleteShopping, favoriteCustomFood, listCustomFoods, listPlans, listRecipes, listShopping, logPlanMeal, updateCustomFood, updateShopping, type CustomFood, type Plan, type Recipe, type ShoppingItem } from './api/catalog'
...
function CatalogScreen({ authenticated }: { authenticated: boolean }) {
  const [foods, setFoods] = useState<CustomFood[]>([]); const [recipes, setRecipes] = useState<Recipe[]>([]); const [name, setName] = useState(''); const [carbs, setCarbs] = useState(''); const [editingId, setEditingId] = useState<string | null>(null); const [message, setMessage] = useState<string | null>(null)
...
function PlannerScreen({ authenticated, today }: { authenticated: boolean; today: string }) {
  ...
  const removePlanItem = async (id: string) => { if (authenticated) await deletePlan(id); else await deleteGuestPlan(id); await load() }; const logPlan = async (plan: Plan) => { if (!authenticated) { await createGuestMealFromPlan(plan, today); setMessage('A tervezett étkezés bekerült a vendégnaplóba.'); return }; await logPlanMeal(plan.id, { idempotency_key: crypto.randomUUID(), local_date: today }); setMessage('A tervezett étkezés bekerült a naplóba.'); }; const toggleShopping = async (item: ShoppingItem) => { const updated = { ...item, checked: !item.checked, updatedAt: new Date().toISOString() }; if (authenticated) await updateShopping(item.id, { checked: updated.checked }); else await saveGuestShopping(updated); await load() }; const removeShopping = async (id: string) => { if (authenticated) await deleteShopping(id); else await deleteGuestShopping(id); await load() }
  return <main className="screen planner-screen"><p className="eyebrow">Napi tervező</p><h1>Tervezés és bevásárlás</h1><section className="catalog-card"><div className="section-heading"><h2>{today}</h2><button className="secondary-action compact-action" onClick={() => void add()}>Terv hozzáadása <Plus size={15} /></button></div><select className="planner-select" value={source} onChange={(event) => setSource(event.target.value)}><option value="">Saját étel vagy recept…</option>{foods.map((item) => <option value={'food:' + item.id} key={'food:' + item.id}>Étel · {item.name}</option>)}{recipes.map((item) => <option value={'recipe:' + item.id} key={'recipe:' + item.id}>Recept · {item.name}</option>)}</select>{plans.length === 0 && <p className="search-state">Még nincs tervezett étkezés erre a napra.</p>}{plans.map((plan) => <div className="catalog-row" key={plan.id}><span><strong>{String(plan.snapshot.name ?? 'Tervezett étkezés')}</strong><small>{formatCarbohydrate(plan.plannedCarbsG)} g CH</small></span><span className="catalog-actions"><button onClick={() => void logPlan(plan)} aria-label="Terv rögzítése a naplóba"><Check size={16} /></button><button onClick={() => void removePlanItem(plan.id)} aria-label="Terv törlése"><Trash2 size={16} /></button></span></div>)}</section><section className="catalog-card"><div className="section-heading"><h2>Bevásárlólista</h2><button className="secondary-action compact-action" onClick={() => void addShopping()}>Tétel <Plus size={15} /></button></div>{shopping.length === 0 && <p className="search-state">Üres lista.</p>}{shopping.map((item) => <div className="catalog-row" key={item.id}><span><strong className={item.checked ? 'shopping-checked' : ''}>{item.name}</strong><small>{item.quantity ?? ''} {item.unit}</small></span><span className="catalog-actions"><button onClick={() => void toggleShopping(item)} aria-label={item.name + ' kipipálása'}><Check size={16} /></button><button onClick={() => void removeShopping(item.id)} aria-label={item.name + ' törlése'}><Trash2 size={16} /></button></span></div>)}</section>{message && <p className="goal-help" role="status">{message}</p>}</main>
}
