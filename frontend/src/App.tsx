import { ArrowLeft, Camera, Check, ChevronRight, CircleUserRound, Minus, Moon, Pencil, Plus, Search, Sparkles, Sun, Trash2, Utensils, X } from './components/icons'
...
import { lookupFoodBarcode, searchFoods, type Food } from './api/foods'
...
import { NutritionScanner } from './camera/NutritionScanner'
import { VisionScanner } from './camera/VisionScanner'
import type { NutritionDraft } from './camera/nutrition'
...
function App() {
  ...
  const quickRecipesQuery = useQuery({ queryKey: ['quick-recipes', authenticated ? 'account' : 'guest'], queryFn: () => authenticated ? listRecipes() : listGuestRecipes(), enabled: !authenticated || authQuery.isSuccess, staleTime: 30_000 })
  ...
  async function logQuickRecipe(recipe: Recipe) {
    try {
      if (authenticated) await logRecipeMeal(recipe.id, { quantity: 1, quantity_unit: 'servings', local_date: selectedDate, meal_category: mealCategory, idempotency_key: crypto.randomUUID() })
      else await createGuestMealFromPlan({ id: crypto.randomUUID(), planDate: selectedDate, mealCategory, foodId: null, customFoodId: null, recipeId: recipe.id, quantity: 1, quantityUnit: 'servings', plannedCarbsG: recipe.carbsPerServingG, snapshot: { name: recipe.name, source: 'recipe', source_id: recipe.id }, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }, selectedDate)
      void queryClient.invalidateQueries({ queryKey: ['meals', selectedDate] }); void queryClient.invalidateQueries({ queryKey: ['goal-summary', selectedDate] }); showSavedToast('Recept mentve a naplóba')
    } catch (error) { setActionError(error instanceof Error ? error.message : 'A recept naplózása nem sikerült.') }
  }
  ...
  return <div className={'app-shell ' + (darkMode ? 'theme-dark' : '')}>
    ...
    {isSheetOpen && <AddMealSheet guestMode={!authenticated} mode={editingMeal ? 'edit' : 'create'} selectedDate={selectedDate} todayDate={todayDate} quickRecipes={quickRecipesQuery.data ?? []} onQuickRecipe={(recipe) => void logQuickRecipe(recipe)} query={query} setQuery={setQuery} selectedFood={selectedFood} setSelectedFood={setSelectedFood} amount={amount} setAmount={setAmount} mealCategory={mealCategory} setMealCategory={setMealCategory} onClose={closeSheet} onAdd={saveMeal} isSaving={isSaving} error={actionError} />}
...
function AddMealSheet({ guestMode, mode, selectedDate, todayDate, quickRecipes, onQuickRecipe, query, setQuery, selectedFood, setSelectedFood, amount, setAmount, mealCategory, setMealCategory, onClose, onAdd, isSaving, error }: { guestMode: boolean; mode: 'create' | 'edit'; selectedDate: string; todayDate: string; quickRecipes: Recipe[]; onQuickRecipe: (recipe: Recipe) => void; query: string; setQuery: (value: string) => void; selectedFood: Food | null; setSelectedFood: (food: Food | null) => void; amount: string; setAmount: (value: string) => void; mealCategory: MealCategory; setMealCategory: (value: MealCategory) => void; onClose: () => void; onAdd: () => void; isSaving: boolean; error: string | null }) {
  ...
  const quickCustomQuery = useQuery({ queryKey: ['quick-custom-foods', guestMode], queryFn: () => guestMode ? listGuestCustomFoods() : listCustomFoods(), enabled: normalizedQuery.length < 2, staleTime: 30_000 })
  ...
  const quickCustomFoods = (quickCustomQuery.data ?? []).filter((item) => item.isFavorite).map((item): Food => ({ id: `custom-${item.id}`, name: item.name, originalName: null, brand: item.brand, barcode: null, source: 'custom', sourceId: item.id, availableCarbs100g: item.availableCarbs100g, servingSizeG: item.servingSizeG, imageUrl: null, language: 'hu', country: null, isGeneric: false, isVerified: true, category: 'custom', categoryLabel: 'Kedvenc saját étel', carbsAvailable: true }))
  const [cameraOpen, setCameraOpen] = useState(false)
  const [cameraMode, setCameraMode] = useState<'barcode' | 'nutrition' | 'vision'>('barcode')
  const [cameraMessage, setCameraMessage] = useState<string | null>(null)
  const [cameraBusy, setCameraBusy] = useState(false)
  const handleBarcode = async (barcode: string) => {
    setCameraBusy(true); setCameraMessage(null)
    try {
      const food = await lookupFoodBarcode(barcode)
      if (!food) { setCameraMessage('Ehhez a vonalkódhoz nincs találat. Próbáld a kézi keresést vagy hozz létre saját ételt.'); return }
      if (!food.carbsAvailable) { setCameraMessage('A termékhez nem érhető el ellenőrzött CH-adat. Nem naplózható automatikusan.'); return }
      setSelectedFood(food); setCameraOpen(false)
    } catch (value) { setCameraMessage(value instanceof Error ? value.message : 'A vonalkódos keresés nem sikerült.') } finally { setCameraBusy(false) }
  }
  const handleNutrition = async (draft: NutritionDraft) => {
    if (draft.basis !== '100g' || draft.carbs100g === null || !Number.isFinite(draft.carbs100g) || draft.carbs100g < 0 || draft.carbs100g > 100) { setCameraMessage('Csak ellenőrzött, 100 g alapú CH-adattal lehet saját ételt létrehozni.'); return }
    setCameraBusy(true); setCameraMessage(null)
    try {
      const now = new Date().toISOString()
      const saved = guestMode ? { id: crypto.randomUUID(), name: draft.name.trim(), brand: null, availableCarbs100g: draft.carbs100g, dietaryFiber100g: draft.fiber100g, servingSizeG: null, notes: 'Kamerás tápérték-felismerés után jóváhagyva', isFavorite: false, createdAt: now, updatedAt: now } satisfies CustomFood : await createCustomFood({ name: draft.name.trim(), available_carbs_100g: draft.carbs100g, dietary_fiber_100g: draft.fiber100g, notes: 'Kamerás tápérték-felismerés után jóváhagyva' })
      if (guestMode) await saveGuestCustomFood(saved)
      setSelectedFood({ id: `custom-${saved.id}`, name: saved.name, originalName: null, brand: saved.brand, barcode: null, source: 'custom', sourceId: saved.id, availableCarbs100g: saved.availableCarbs100g, servingSizeG: saved.servingSizeG, imageUrl: null, language: 'hu', country: null, isGeneric: false, isVerified: true, category: 'custom', categoryLabel: 'Saját étel', carbsAvailable: true })
      setCameraOpen(false)
    } catch (value) { setCameraMessage(value instanceof Error ? value.message : 'A saját étel mentése nem sikerült.') } finally { setCameraBusy(false) }
  }
  const amountGrams = parseAmountInput(amount)
