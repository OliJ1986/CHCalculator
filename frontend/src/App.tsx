...
function PlannerScreen({ authenticated, today }: { authenticated: boolean; today: string }) {
  ...
  const [plans, setPlans] = useState<Plan[]>([]); const [shopping, setShopping] = useState<ShoppingItem[]>([]); const [foods, setFoods] = useState<CustomFood[]>([]); const [recipes, setRecipes] = useState<Recipe[]>([]); const [meals, setMeals] = useState<Meal[]>([]); const [source, setSource] = useState(''); const [message, setMessage] = useState<string | null>(null)
  ...
  const generate = async () => { try { if (authenticated) await generateShopping(weekStart, endDate(weekStart)); else { const grouped = new Map<string, number>(); plans.forEach((plan) => { const name = String(plan.snapshot.name ?? 'Élelmiszer'); grouped.set(name, (grouped.get(name) ?? 0) + plan.quantity) }); for (const [name, quantity] of grouped) await saveGuestShopping({ id: crypto.randomUUID(), name, quantity, unit: 'g', checked: false, source: 'planner', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }) }; await load(); setMessage('A heti bevásárlólista frissítve.') } catch (error) { setMessage(error instanceof Error ? error.message : 'A lista generálása nem sikerült.') } }
