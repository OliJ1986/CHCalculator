...
import { clearGuestData, createGuestMeal, deleteGuestMeal, exportGuestData, getGuestSummary, guestWindowAllows, listGuestMeals, saveGuestGoal, updateGuestMeal } from './storage/guestStore'
...
function App() {
  ...
  const authReady = authQuery.isSuccess || authQuery.isError
  const mealsQuery = useQuery({ queryKey: ['meals', selectedDate, authenticated ? 'account' : 'guest'], queryFn: ({ signal }) => authenticated ? listMeals(selectedDate, signal) : listGuestMeals(selectedDate, todayDate), enabled: authReady, staleTime: 15_000 })
  const goalQuery = useQuery<GoalSummary>({ queryKey: ['goal-summary', selectedDate, authenticated ? 'account' : 'guest'], queryFn: ({ signal }) => authenticated ? getGoalSummary(selectedDate, signal) : getGuestSummary(selectedDate, todayDate), enabled: authReady, staleTime: 15_000 })
