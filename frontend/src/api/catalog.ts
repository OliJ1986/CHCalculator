...
export type Recipe = { id: string; name: string; instructions: string | null; prepMinutes: number | null; notes: string | null; servings: number; totalWeightG: number | null; totalCarbsG: number; carbsPerServingG: number; isFavorite: boolean; ingredients: RecipeIngredient[]; createdAt: string; updatedAt: string }
...
const mapRecipe = (x: any): Recipe => ({ id:x.id,name:x.name,instructions:x.instructions,prepMinutes:x.prep_minutes,notes:x.notes,servings:x.servings,totalWeightG:x.total_weight_g,totalCarbsG:x.total_carbs_g,carbsPerServingG:x.carbs_per_serving_g,isFavorite:x.is_favorite,ingredients:(x.ingredients ?? []).map((i:any)=>({id:i.id,foodId:i.food_id,customFoodId:i.custom_food_id,quantityG:i.quantity_g,calculatedCarbsG:i.calculated_carbs_g,snapshot:i.snapshot,position:i.position})),createdAt:x.created_at,updatedAt:x.updated_at })
