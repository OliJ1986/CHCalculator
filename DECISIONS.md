...
- D35: A Railway szolgáltatás Root Directoryja a parancsok munkakönyvtára. A staging backend `/backend` és frontend `/frontend` konfigurációi ezért közvetlen parancsokat használnak; a gyökér `railway.toml` csak repo-root fallbackként tartja meg a `cd backend` előtagot. A három konfiguráció eltérését TOML-regressziós teszt védi.
- D36: A backend Nixpacks felismerését a `backend/nixpacks.toml` explicit Python providerrel, Python 3.12-vel és `requirements` csomagkezelővel rögzíti. A `pyproject.toml` teszt/lint konfiguráció marad, nem telepítési forrás.
- D37: A frontend Railway build parancsa nem futtat második `npm ci`-t. A Nixpacks install fázis kezeli a lockfile telepítését, a build fázis csak `npm run build`; ez elkerüli a `node_modules/.cache` EBUSY eltávolítását.
